...
def compute_item_likelihoods(rows, label_index, laplace=1.0):
    ...
    label_index: index of label column (0/1 depression)
    laplace: Laplace smoothing factor

    Returns:
      p1[j] = P(feature_j = 1 | depression = 1)  (softened)
      p0[j] = P(feature_j = 1 | depression = 0)  (softened)
      prior = fixed prior P(depression = 1)
    """
    n = len(rows)
...
def naive_bayes_posterior_for_row(answers, p1, p0, prior):
    ...
    prior: P(depression=1)

    Returns: posterior P(depression=1 | answers) using Naive Bayes in log-odds space.
    """
...
def update_posterior(pi, p1_q, p0_q, response):
    """
    pi: current P(depression=1 | history)
    p1_q: P(x=1 | depression=1) for this question
    p0_q: P(x=1 | depression=0) for this question
    response: 0 or 1
    """
    if response == 1:
        L1 = p1_q
...
def label_category(pi):
    ...
    """
    if pi >= 0.43:
        return "High possibility of Depressive Disorder, Consult a Physician"    
    elif pi <= 0.33:
        return "Low possibility of Depressive Disorder"
    else:
        return "Moderate possibility of Depressive Disorder, Consult a Physician"

#############################################
# 5. Interactive adaptive depression session
#############################################

def run_adaptive_depression_session():
    ...
    print(f"Using prior P(depression=1) = {prior:.3f}")
    ...
    print(f"\nFinal probability of depression = {pi:.3f}")
...
def simulate_adaptive_depression():
    ...
    print("\n===== ADAPTIVE DEPRESSION SURVEY SIMULATION RESULTS =====")
...
def visualize_naive_bayes_calibration(num_bins=10):
    ...
    plt.ylabel("Empirical fraction with depression")
    plt.title("Calibration curve – Naive Bayes (Depression)")
    plt.legend()
    plt.grid(True)
...
def main():
    ...
    # run_adaptive_depression_session()

    # 2) Or run the simulator:
    simulate_adaptive_depression()
