...
def logistic_regression_trainer():
  with open('csv-files/Mental-Illness-Train-PTSD.csv', mode='r', newline='') as file:
...
def logistic_regression_tester(theta_list):
  with open('csv-files/Mental-Illness-Test-PTSD.csv', mode='r', newline='') as file:
    csv_reader = csv.reader(file)
    header = next(csv_reader)
...
def plot_calibration_curve(bin_centers, avg_pred, avg_obs):
    ...
    plt.title("Calibration Curve PTSD Logistic Regression")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
def give_user_prediction(theta_list):
  ...
  if sigmoid >= 0.85:
    print("Very High Possibility of PTSD")
  elif sigmoid >= 0.6:
    print("High Possibility of PTSD")
  elif sigmoid >= 0.3:
    print("Moderate Possibility of PTSD")
  else:
    print("Low Possibility of PTSD")
